        ...
        .then(data => {
          ...
          inscritosParaInscrever.forEach((id, index) => {
            ...
            .then(data => {
              ...
              console.log(`Inscrito ${id} transferido com sucesso!`);
            })
            .catch(error => {
              statusDiv.innerHTML = `Erro ao inscrever ${id}: ${error.message}`;
              console.error(`Erro ao inscrever ${id}: ${error.message}`);
            });
          });
          statusDiv.innerHTML = `Transferência de inscritos concluída!`;
        })
        .catch(error => {
          statusDiv.innerHTML = `Erro ao listar inscritos do canal de destino: ${error.message}`;
          console.error(`Erro ao listar inscritos do canal de destino: ${error.message}`);
        });
    })
    .catch(error => {
      statusDiv.innerHTML = `Erro ao listar inscritos do canal de origem: ${error.message}`;
      console.error(`Erro ao listar inscritos do canal de origem: ${error.message}`);
    });
});